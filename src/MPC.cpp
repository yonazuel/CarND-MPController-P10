...
// We also define weights for the cost function
double w_v = 1.0;
double w_epsi = 10.0;
double w_cte = 1.0;
double w_delta = 500.0;
double w_a = 1.0;
double w_delta_dot = 25.0;
double w_a_dot = 1.0;
...
AD<double> eval_poly_der(Eigen::VectorXd coeffs, AD<double> x) {
  AD<double> res = 0.0;
  for(int p = 1; p < coeffs.size(); p++) {
    res += p * coeffs[p] * CppAD::pow(x, p-1);
  }
  return res;
}
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(int t = 1; t < N; t++) {
      ...
      AD<double> f0 = eval_poly(coeffs, x0);
      AD<double> psides0 = CppAD::atan(eval_poly_der(coeffs, x0));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for(int t = 0; t < N; t++) {
    x1.push_back(solution.x[x_start + t+1]);
  }

  for(int t = 0; t < N; t++) {
    x1.push_back(solution.x[y_start + t+1]);
  }
