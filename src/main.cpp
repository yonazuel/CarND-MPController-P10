      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // We calculate the cross track error and orientation error
          double cte = polyeval(coeffs, 0);
          double epsi = -atan(coeffs[1]);

          // We create the vector for the current vector
          Eigen::VectorXd state(6);
          double v_kph = v * 1.60934;
          double x_vehicle = v_kph * 100 / (1000*3600); //this moves forward 100ms at v_kph to correct latency
          double y_vehicle = 0;
          double psi_vehicle = 0;
          state << x_vehicle, y_vehicle, psi_vehicle, v, cte, epsi;
          ...
          next_x_vals.resize(ptsx_vehicle.size());
          next_y_vals.resize(ptsx_vehicle.size());
